...
using reader_t = raw_ml_reader<8192>;
...
void dump_shape(NSVGshape* shape) {
    if (shape == nullptr) {
        printf("\t<null>\n");
        return;
    }
...
void dump_parse_res(svg_parse_result& p) {
    printf("new\n");
...
void dump_parse_res(NSVGparser& p) {
    printf("old\n");
...
static gfx_result svg_parse_sub_elems(svg_parse_result& ctx);
...
static const char* svg_local_name(reader_t& s) {
    const char* sz = s.value();
    if (sz == nullptr) {
        return nullptr;
    }
    while (*sz && *sz != ':') {
...
static gfx_result svg_skip_elem(svg_parse_result& ctx) {
    reader_t& s = *ctx.reader;
    if (s.node_type() != ml_node_type::element) {
        return gfx_result::invalid_state;
    }
    int i = 0;
    while (i >= 0 && s.read()) {
        int nt = (int)s.node_type();
        switch (s.node_type()) {
    ...
    if (i == -1 && s.node_type() == ml_node_type::element_end && s.read()) {
        return gfx_result::success;
    }
    return gfx_result::invalid_format;
}
...
static gfx_result svg_parse_gradient_elem(svg_parse_result& ctx, signed char type) {
    ...
    ctx.gradients = grad;
    if (s.skip_to_end_element() && s.read()) {
        return gfx_result::success;
    }
    return gfx_result::invalid_format;
}
...
static gfx_result svg_parse_rect_elem(svg_parse_result& p) {
    ...
    float ry = -1.0f;
    int i;
    gfx_result res = svg_push_attr(p);
    if (res != gfx_result::success) {
        return res;
    }
    while (s.node_type() == ml_node_type::attribute) {
    ...
    svg_pop_attr(p);
    if (s.skip_to_element_children()) {
        gfx_result res = svg_parse_sub_elems(p);
        if (res != gfx_result::success) {
            return res;
        }
    }
    if (s.skip_to_end_element() && (s.is_empty_element() || 0 == strcmp("rect", s.value())) && s.read()) {
        return gfx_result::success;
    }
    return gfx_result::invalid_format;
}
...
static gfx_result svg_parse_circle_elem(svg_parse_result& p) {
    ...
    if (s.skip_to_end_element() && (s.is_empty_element() || 0 == strcmp("circle", s.value())) && s.read()) {
        return gfx_result::success;
    }
    return gfx_result::invalid_format;
}
...
static gfx_result svg_parse_ellipse_elem(svg_parse_result& p) {
    ...
    if (s.skip_to_end_element() && (s.is_empty_element() || 0 == strcmp("ellipse", s.value())) && s.read()) {
        return gfx_result::success;
    }
    return gfx_result::invalid_format;
}
...
static gfx_result svg_parse_line_elem(svg_parse_result& p) {
    ...
    if (s.skip_to_end_element() && (s.is_empty_element() || 0 == strcmp("line", s.value())) && s.read()) {
        return gfx_result::success;
    }
    return gfx_result::invalid_format;
}
...
static gfx_result svg_parse_path(svg_parse_result& p) {
    reader_t& s = *p.reader;
    if (s.node_type() != ml_node_type::attribute_content) {
        printf("parse_path not on attribute content");
        return gfx_result::invalid_state;
    }
    gfx_result res;
    const char* str = s.value();
    char cmd = '\0';
    float args[10];
    int nargs;
    int rargs = 0;
    ...
    while (*str) {
        ...
        if (cmd != '\0' && nsvg__isCoordinate(item)) {
            if (nargs < 10) {
                args[nargs] = (float)nsvg__atof(item);
                if(((int)args[nargs])==239 || ((int)args[nargs])==39) {
                    printf("arg: %f\n",args[nargs]);
                }
                ++nargs;
            }
    ...
    if (!s.read() || s.node_type() == ml_node_type::attribute_content) {
        printf("path attribute too long\n");
        return gfx_result::invalid_format;
    }
    if (p.plist != nullptr) {
        res = svg_add_shape(p);
...
static gfx_result svg_parse_g_elem(svg_parse_result& ctx) {
    printf("parse <g>\n");
    reader_t& s = *ctx.reader;
    ...
    svg_parse_attribs(ctx);
    // move to sub elements
    if (s.skip_to_element_children()) {
        res = svg_parse_sub_elems(ctx);
        if (res != gfx_result::success) {
            return res;
        }
        if (s.skip_to_end_element() && (s.is_empty_element() || 0 == strcmp("g", s.value())) && s.read()) {
            svg_pop_attr(ctx);
            return gfx_result::success;
        }
    }
...
static gfx_result svg_parse_defs_elem(svg_parse_result& ctx) {
    printf("parse <defs>\n");
    reader_t& s = *ctx.reader;
    gfx_result res = gfx_result::success;
    if (!s.read()) {
    ...
    while ((s.node_type() == ml_node_type::attribute || s.node_type() == ml_node_type::attribute_content || s.node_type() == ml_node_type::attribute_end) && s.read())
        ;
    if (s.node_type() == ml_node_type::element_end && s.is_empty_element()) {
        return s.read() ? gfx_result::success : gfx_result::invalid_format;
    }
    // move to sub elements
    if (s.skip_to_element_children()) {
        while (true) {
            while (s.node_type() != ml_node_type::element && s.node_type() != ml_node_type::element_end && s.read())
                ;
            if (s.node_type() == ml_node_type::element_end) {
                break;
            }
            strncpy(ctx.lname, svg_local_name(s), sizeof(ctx.lname));

            if (0 == strcmp("linearGradient", ctx.lname)) {
                printf("parse <linearGradient>\n");
                res = svg_parse_gradient_elem(ctx, NSVG_PAINT_LINEAR_GRADIENT);  // linear
                if (res != gfx_result::success) {
                    return res;
                }
            } else if (0 == strcmp("radialGradient", ctx.lname)) {
                printf("parse <radialGradient>\n");
                res = svg_parse_gradient_elem(ctx, NSVG_PAINT_RADIAL_GRADIENT);
                if (res != gfx_result::success) {
                    return res;
                }
            } else if (0 == strcmp("stop", ctx.lname)) {
        ...
        if (s.skip_to_end_element() && (s.is_empty_element() || 0 == strcmp("defs", s.value())) && s.read()) {
            return gfx_result::success;
        }
    }
    return gfx_result::invalid_format;
...
static gfx_result svg_parse_sub_elems(svg_parse_result& ctx) {
    ...
    while (true) {
        ...
        strncpy(ctx.lname, svg_local_name(s), sizeof(ctx.lname));
        if (0 == strcmp("path", ctx.lname)) {
            res = svg_parse_path_elem(ctx);
            if (res != gfx_result::success) {
                return res;
            }
        } else if (0 == strcmp("rect", ctx.lname)) {
            res = svg_parse_rect_elem(ctx);
            if (res != gfx_result::success) {
                return res;
            }
        } else if (0 == strcmp("circle", ctx.lname)) {
            res = svg_parse_circle_elem(ctx);
            if (res != gfx_result::success) {
                return res;
            }
        } else if (0 == strcmp("ellipse", ctx.lname)) {
            res = svg_parse_ellipse_elem(ctx);
            if (res != gfx_result::success) {
                return res;
            }
        } else if (0 == strcmp("line", ctx.lname)) {
            res = svg_parse_line_elem(ctx);
            if (res != gfx_result::success) {
                return res;
            }
        } else if (0 == strcmp("polyline", ctx.lname)) {
            res = svg_parse_poly_elem(ctx, 0);
            if (res != gfx_result::success) {
                return res;
            }
        } else if (0 == strcmp("polygon", ctx.lname)) {
            res = svg_parse_poly_elem(ctx, 1);
            if (res != gfx_result::success) {
                return res;
            }
        } else if (0 == strcmp("g", ctx.lname)) {
            res = svg_parse_g_elem(ctx);
            if (res != gfx_result::success) {
                return res;
            }
        } else if (0 == strcmp("defs", ctx.lname)) {
            res = svg_parse_defs_elem(ctx);
            if (res != gfx_result::success) {
                return res;
            }
        } else if (0 == strcmp("svg", ctx.lname)) {
            res = svg_parse_svg_elem(ctx);
            if (res != gfx_result::success) {
                return res;
            }
        } else if (0 == strcmp("linearGradient", ctx.lname)) {
            printf("parse <linearGradient>\n");
            res = svg_parse_gradient_elem(ctx, NSVG_PAINT_LINEAR_GRADIENT);  // linear
            if (res != gfx_result::success) {
                return res;
...
static gfx_result svg_parse_document(reader_t& s, svg_parse_result* p) {
    p->reader = &s;
    while (s.node_type() != ml_node_type::element && s.read())
        ;
    if (s.node_type() != ml_node_type::element) {
        return gfx_result::invalid_format;
    }
    if (0 != strcmp(svg_local_name(s), "svg")) {
        return gfx_result::invalid_format;
    }
    return svg_parse_svg_elem(*p);
}
...
void svg_doc::draw(float scale, srect16 rect, void(read_callback)(int x, int y, unsigned char* r, unsigned char* g, unsigned char* b, unsigned char* a, void* state), void* read_callback_state, void(write_callback)(int x, int y, unsigned char r, unsigned char g, unsigned char b, unsigned char a, void* state), void* write_callback_state) {
    if (initialized()) {
